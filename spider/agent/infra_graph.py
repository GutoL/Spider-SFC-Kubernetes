...
class InfrastructureGraph(nx.MultiGraph):
  ...
  def __init__(self, nodes_data, links_data):
    super(InfrastructureGraph, self).__init__()
    self.components = ['cpu','memory','storage']
    self.add_attributes_from_infra(nodes_data, links_data)

  def add_attributes_from_infra(self, nodes_data, links_data):
    all_nodes = [(node['id'],node) for node in nodes_data]
    self.add_nodes_from(all_nodes)
        
    all_edges = [(edge['source']['id'], edge['destination']['id'], edge) for edge in links_data]
